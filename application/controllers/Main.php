...
class Main extends CI_Controller
{
	...
	public function web_comp($param1 = null, $param2 = null)
	{
		...
		//$data['active'] = 'web_comp';
			...
			// idate(format)// 
				...
				case 'list':
					$view = "list";
					$this->data['multiListingData'] = $this->model_users->get_all();
			...
			// echo '<pre>';print_r($this->data);die;
				...
				case 'in_progress':
					...
					$this->data['multiListingData'] = $this->model_users->select_all_in_progress_list("complaint");

					break;

				case 'completed':
					$view = "completed_list";
					$this->data['multiListingData'] = $this->model_users->select_all_completed_list("complaint");
				...
				case 'under_review':
					...
					$this->data['multiListingData'] = $this->model_users->select_all_under_review_list("complaint");
	...
	public function admin_complaint($param1 = null, $param2 = null)
	{
		if ($this->session->userdata('is_logged_in')) {
			$data = '';
		//$data['active']		=	'admin_complaint';

			switch ($param1) {

				case 'add':
					$view = 'add_comp';
			//$data1['active']		=	'admin_complaint';
					if ($_POST) {
						echo $data['c_number'] = $this->input->post('c_number');
						echo $data['c_details'] = $this->input->post('c_details');
						$decoded_string = base64_decode($this->input->post('image_path'));
						$image_path = './uploads/map/' . $data['c_number'] . '.jpeg';
						$data['image_path'] = $image_path; //$this->model_users->upload('image_path');
						file_put_contents($image_path, $decoded_string);
			...
			// idate(format)// 
						...
						$Push->setMessage($data);
						$Firebase->send($data['token_id'], $data['status']);
						redirect('main/admin_complaint/pending');
			...
			// idate(format)// 
						...
						$Push->setMessage($data);
						$Firebase->send($data['token_id'], $data['status']);
						redirect('main/admin_complaint/in_progress');
			...
			// idate(format)// 
						...
						$Push->setMessage($data);
						$Firebase->send($data['token_id'], $data['status']);
						redirect('main/admin_complaint/under_review_list');
					}
					break;
				...
				case 'cdelete':
					$view = "list";
					$this->model_users->doDeleteSingleRecord('complaint', 'c_id', $param2);
					redirect('main/admin_complaint/completed');
					break;
				case 'pdelete':
					$view = "list";
					$this->model_users->doDeleteSingleRecord('complaint', 'c_id', $param2);
				...
				case 'udelete':
					$view = "list";
					$this->model_users->doDeleteSingleRecord('complaint', 'c_id', $param2);
					redirect('main/admin_complaint/under_review');
					break;
	...
	public function login_validation()
	{
		...
		if ($this->form_validation->run()) {
			...
			$data = array(
				'mobilenumber' => $this->input->post('mobilenumber'),
				'is_logged_in' => 1,
				'district' => $user->district_slug,
				'tma_district' => $user->district_tma_slug,
			);
